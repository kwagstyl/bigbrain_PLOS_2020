...
# make tmpdir
my $tmpdir = &tempdir( "flipyz-XXXXXX", TMPDIR => 1, CLEANUP => 1 );
...
&run( 'mincresample', '-quiet', '-clob', '-unsigned', '-short',
      ...
      '-start', $sx, $sz, $sy, '-nelements', $nx, $nz, $ny,
      '-step', $dx, $dz, $dy, '-range', 0, 65535, '-keep_real_range', $in, $out );
&run( 'mv', '-f', $out, $input );
