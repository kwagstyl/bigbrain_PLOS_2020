...
# make tmpdir
my $tmpdir = &tempdir( "flipxz-XXXXXX", TMPDIR => 1, CLEANUP => 1 );
...
&run( 'mincresample', '-quiet', '-clob', '-unsigned', '-short',
      ...
      '-start', $sz, $sy, $sx, '-nelements', $nz, $ny, $nx,
      '-step', $dz, $dy, $dx, '-range', 0, 65535, '-keep_real_range', $in, $out );
&run( 'mv', '-f', $out, $input );
